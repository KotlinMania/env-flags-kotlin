...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`env-flags`](https://crates.io/crates/env-flags).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `env-flags` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Boolean flag parsing from environment variables
...
Original work copyrighted by the upstream `env-flags` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
