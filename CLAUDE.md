# Claude Code Project Instructions — env-flags-kotlin

## Project Overview

This is **env-flags-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`env-flags`](https://crates.io/crates/env-flags).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/env-flags/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/env-flags>
