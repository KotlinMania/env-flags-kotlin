...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `EnvFlags.swiftmodule/`
...
//      `import EnvFlags` above would fail at compile time.
//
//   2. The static archive `libEnvFlags.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "EnvFlags" }` and
//      `flattenPackage = "io.github.kotlinmania.envflags"` configuration in
...
//      via the `EnvFlagsLibrary` product.
//
...
final class EnvFlagsExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "EnvFlags swift module imported cleanly")
    }
}
